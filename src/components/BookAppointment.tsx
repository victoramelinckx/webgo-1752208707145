...
export const BookAppointment: FC<BookAppointmentProps> = ({
  ...
  const handleForm = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const nameInput = currentForm.elements.namedItem(
      "name"
    ) as HTMLInputElement | null;
    const emailInput = currentForm.elements.namedItem(
      "email"
    ) as HTMLInputElement | null;
    const phoneInput = currentForm.elements.namedItem(
      "phone"
    ) as HTMLInputElement | null;
    const mensajeInput = currentForm.elements.namedItem(
      "mensaje"
    ...
    if (!projectId) {
      console.error(
        "BookAppointment: projectId is missing. Cannot submit form correctly."
      );
      notifyError();
      setEnviando(false);
      return;
    ...
    try {
      const response = await fetch(
        `https://webgo-generator.vercel.app/${formPostUrl}`,
        {
          method: "POST",
          headers: {
